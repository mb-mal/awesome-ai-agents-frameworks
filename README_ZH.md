...
192,499 星标 · 32,978 分叉 · 595 贡献者 · 5,465 问题 · TypeScript · MIT

    - 跨操作系统兼容性：支持Windows、macOS、Linux等任意OS
    - 跨平台运行能力：适用于桌面、移动、服务器等所有平台
    - 个性化AI助手：用户专属的智能交互体验
    - 龙虾式轻量架构：高效、模块化且资源占用低
...
18,483 星标 · 2,702 分叉 · 45 贡献者 · 401 问题 · Python · MIT

    - 超轻量级 AI 代理内核
    - 支持多个提供商
    - 自定义技能与 cron 任务
    - 多通道通信（Telegram、Slack 等）
...
535 星标 · 40 分叉 · 5 贡献者 · 33 问题 · Rust · MIT
...
64,165 星标 · 8,060 分叉 · 148 贡献者 · 78 问题 · Python · MIT
    ...
    - 多代理协作架构：通过多个角色代理（如产品经理、架构师、工程师）模拟软件公司分工协作。
    - 标准化操作流程（SOP）：定义明确的代理行为规范，确保高效一致的软件开发流程。
    - 自然语言编程支持：用户输入自然语言需求，即可自动生成完整软件产品。
    - 端到端软件交付：输出包括PRD、系统设计、代码、测试用例及部署文档的全栈产物。
...
54,535 星标 · 8,216 分叉 · 532 贡献者 · 586 问题 · Python · CC-BY-4.0
    ...
    - 多代理协作框架，支持多个AI代理进行对话与任务协作
    - 集成多种大型语言模型（LLM），兼容OpenAI、Azure等提供商
    - 内置工具调用与代码执行能力，实现复杂工作流自动化
    - 可扩展的代理架构，支持人类参与和自定义代理行为
...
37,839 星标 · 5,006 分叉 · 390 贡献者 · 565 问题 · Python · Apache-2.0
    ...
    - 多代理协作架构：支持构建分布式多智能体系统，实现高效任务分工与协调。
    - 增量学习机制：代理在每次交互中实时更新知识库，实现持续优化与能力提升。
    - 交互反馈循环：自动捕获交互数据，形成闭环学习路径，确保系统逐步改进。
    - 自适应优化引擎：基于历史交互动态调整策略，提升整体性能与鲁棒性。
...
25,411 星标 · 2,289 分叉 · 197 贡献者 · 369 问题 · Python · Apache-2.0

    - 极简架构：最小依赖，轻量级库设计，便于快速集成。
    - 代码驱动推理：代理通过生成和执行Python代码进行思考与问题求解。
    - 简单API接口：提供直观的接口，轻松构建和部署智能代理。
    - 沙箱执行环境：支持安全的代码执行机制，避免潜在风险。
...
25,304 星标 · 3,359 分叉 · 216 贡献者 · 180 问题 · Python · Apache-2.0
...
24,698 星标 · 4,301 分叉 · 272 贡献者 · 289 问题 · Python · MIT
    ...
    - 图结构化代理构建：将语言代理建模为图，支持节点和边定义工作流。
    - 弹性执行机制：内置重试、错误恢复和持久化，确保代理鲁棒性。
    - 状态管理支持：维护跨步骤的状态，启用复杂多代理交互。
    - 循环与分支控制：允许条件分支、循环执行和人类干预，提升代理灵活性。
...
2,760 星标 · 456 分叉 · 51 贡献者 · 101 问题 · Python · MIT
    ...
    - 支持同步和异步客户端操作
    - 流式响应生成，实现实时交互
    - 集成Pydantic模型，提供类型安全和数据验证
    - 内置工具调用功能，支持模型与外部工具交互
